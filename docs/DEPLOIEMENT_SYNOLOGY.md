...
Depuis l'intégration de l'app `decoupe` (chiffrage laser/jet d'eau, import
DXF/DWG), le premier `docker compose up -d --build` peut prendre nettement
plus longtemps sur un NAS ARM : la bibliothèque Shapely n'a pas toujours de
roue (wheel) précompilée pour cette architecture, auquel cas `pip` la
compile depuis les sources (le `Dockerfile` installe `libgeos-dev` pour
cela). Les reconstructions suivantes réutilisent le cache Docker et sont
bien plus rapides.

## Sauvegarde des données

Les données vivent dans deux volumes Docker : `erp_postgres_data`
(PostgreSQL) et `erp_media_data` (fichiers DXF/DWG déposés dans l'app
`decoupe`). Pour une sauvegarde simple de la base :
